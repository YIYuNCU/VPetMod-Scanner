...
首批规则来自 `Test/` 下捕获的 **PxBridge** 家族三个样本：

| 工坊条目 | MOD 名 / 作者 | 被替换的插件 |
|---|---|---|
| 3803401951 | 刮刮卡 v1.0.4 / 步眠 | VPet.Plugin.ScratchCard.dll |
| depot …3678898894967785038 | SmartPet / SMZ | SmartPet.dll |
| depot …4637760495230589419 = **3803426816** | 天籁之音！/ 小铃铛·MadeSpark | VPet.Plugin.TianLaiZhiYin.dll |
...
- **规则联动**：`NET-KNOWN-C2`（critical）——任何被测文件**明文**出现 `bvdpp.top`（子域名也算）直接判恶意（原始样本里它在加密节内静态不可见；明文出现=新变种漏加密或配套投放物）。动态侧 `analyze-linux.py` 命中该域名同样直接判 malicious 并在报告标注。
...
> **为什么默认连 `clean` 也引爆**：静态"干净"只代表没命中已知结构特征——全新家族、只在 `DllMain` 里动手、或加密到静态看不出的载荷都可能是这种。所以只要样本里有**原生 PE**（能被 `LoadLibrary` 触发的代码），无论静态判什么都进动态复核一次；纯资源包 / 仅托管插件（没有原生 PE，本套原生 harness 无从引爆）会自动跳过、不浪费引爆。引爆目标：优先结构命中的 boot/载荷，否则兜底挑一个原生 PE（`native/` 目录下的 `.dll` 优先）。每次引爆用完即删（Wine 前缀 ~770M/次，worker `finally` 清理）。
...
>
> **托管 .NET 插件的明文外联不需要动态分析**：静态扫描会直接提取并显示在「🔗 外联地址（静态提取）」板块（`report.network_indicators`），如 `NET-ENDPOINT`（URL/域名/IP）与 `NET-EMBEDDED-SECRET`（`sk-`/`ghp_` 等内嵌密钥，页面显示打码值）。
...
- 路径里的 `!` 表示进入压缩包内部。
- MOD 根目录 = 带 `vupmod#` 行的 `info.lps` 所在目录，与 VPet 自己的判定一致（动画目录里的 `info.lps` 不算 MOD 根）。
...
误报基线（2026-09-19 实测）：本机工坊 49 个 MOD 加 VPet 自带 mod 目录，共约 15000 个文件、146 个 PE，全部判为干净。内嵌 Harmony/MonoMod 的正常 MOD 会命中 `GEN-STUB-NATIVE-EXEC`，所以这条规则只给 medium。
